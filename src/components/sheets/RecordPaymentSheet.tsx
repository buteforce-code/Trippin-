...
export function RecordPaymentSheet({ target }: RecordPaymentSheetProps) {
  ...
  const isExtra = target.owed <= 0
  const [amount, setAmount] = useState(isExtra ? '' : String(target.owed))

  const parsed = parseInt(amount.replace(/[^\d]/g, ''), 10) || 0
  const overpay = !isExtra && parsed > target.owed

  const handleSave = async () => {
    if (!parsed) return
    await recordPayment.mutateAsync({ memberIndex: target.memberIndex, amount: parsed })
    closeRecordSheet()
  }

  return (
    <SheetShell onClose={closeRecordSheet} zIndex={42} ariaLabel={isExtra ? `Add extra for ${target.name}` : `Record ${target.name}'s payment`}>
      <div style={{ fontSize: 20, fontWeight: 800, fontFamily: "'Baloo 2',sans-serif", marginBottom: 4 }}>
        {isExtra ? `Add extra for ${target.name}` : `Record ${target.name}'s payment`}
      </div>
      <div style={{ fontSize: 12.5, color: MUTED_TEXT, fontWeight: 600, marginBottom: 16 }}>
        {isExtra ? 'Top up the pool with extra money · logged against the pool' : `${fmt(target.owed)} still owed · logged against the pool`}
      </div>
      ...
      <div role="status" aria-live="polite">
        {overpay && (
          <div style={{ fontSize: 11.5, color: 'var(--primary-d)', fontWeight: 700, marginBottom: 12 }}>
            {fmt(parsed - target.owed)} of this is extra into the pool.
          </div>
        )}
      </div>
      ...
      >
        {isExtra ? 'Add extra' : 'Record payment'}
      </button>
    </SheetShell>
  )
}
